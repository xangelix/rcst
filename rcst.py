...
add.add_argument("dst_route")
add.add_argument("dst_dir")

remove = subparser.add_parser("remove")
remove.add_argument("dst_route")
remove.add_argument("dst_dir")

clean = subparser.add_parser("clean")
clean.add_argument("url")
...
rc_copy_proc = subprocess.run(
    ["rclone", "copy", file_path, f"{dst_route}{dst_dir}"], stdout=subprocess.PIPE
)

rc_link_proc = subprocess.run(
    ["rclone", "link", f"{dst_route}{dst_dir}/{basename}"], stdout=subprocess.PIPE
)
...
if direct_link:
    rc_link = rc_link.replace(
        "https://drive.google.com/open?id=", "https://drive.google.com/uc?id="
    )
